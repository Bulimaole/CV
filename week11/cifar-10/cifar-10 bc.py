...
# 3.使用函数tf.nultiply()计算权重L2 loss与w1的乘机，并赋值给weights_loss
# 4.使用函数tf.add_to_collection()将最终的结果放在名为losses的集合里面，方便后面计算神经网络的总统loss，
...
images_train,labels_train=Cifar10_data_bc.inputs(data_dir=data_dir,batch_size=batch_size,distorted=True)
images_test,labels_test=Cifar10_data_bc.inputs(data_dir=data_dir,batch_size=batch_size,distorted=None)

#创建x和y_两个placeholder，用于再训练或评估时提供输入的数据和对应的标签值
#要注意的是，由于以后定义全连接网络的时候用到了batch——size，所以x中，第一个参数不应该是None，而应该是batch_size
...
#因为要进行全连接层的操作，所以这里使用tf.reshape()函数将pools2输出变成一维向量，并使用get_shape()函数获取扁平化之后的长度
...
#函数tf.nn.in_top_k()用来计算输出结果中top_k的准确率，函数默认的k值是1，即top_1的准确率，也就是输出分类准确率最高时的数值
