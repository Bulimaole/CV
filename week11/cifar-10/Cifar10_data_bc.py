...
#定义一个读取Cifar-10的函数read cifar10（），这个函数的目的是读取目标文件夹里的内容
def read_cifar10(file_queue):
    ...
    record_bytes=tf.decode_raw(value,tf.uint8)  #读取到文件以后，将读取到的文件内容从字符串形式解析为图像对应的像素组数

    #因为该数组第一个元素是标签，所以我们使用strided slice()函数将标签提取出来，并且使用tf.cast()函数将这一个标签转换成int32的数值形式
    result.label=tf.cast(tf.strided_slice(record_bytes,[0],[label_bytes]),tf.int32)

    #剩下的元素再分割出来，这些就是图片数据，因此这些数据在数据集里面存储的形式是depth * height * width，将这种格式转换成[depth,height,width]
    #这一步是将一维数据转成3维数据
    depth_major=tf.reshape(tf.strided_slice(record_bytes,[label_bytes],[label_bytes + image_bytes]),
                           [result.depth,result.height,result.width])

    #将之前分割好的图片数据使用tf.transpose()函数转换成高度信息、宽度信息、深度信息这样的顺序
...
def inputs(data_dir,batch_size,distorted):    #这个函数就对数据进行预处理---对图像数据是否进行增强进行判断，并作出相应的操作
    ...
    read_input=read_cifar10(file_queue)    #更具已有的文件队列使用已经定义好的文件读取函数read cifar10()读取队列中的文件
    ...
    if distorted !=None:    #如果预处理函数中的distorted参数不为空值，就代表进行图片增强处理
        cropped_image=tf.random_crop(reshaped_image,[24,24,3])    #首先将预处理的图片进行剪切，使用tf.radom_crop()函数

        flipped_image=tf.image.random_flip_left_right(cropped_image)    #将剪切好的图片进行左右翻转，使用tf.image.random_flip_left_right()函数

        adjusted_brightness=tf.image.random_brightness(flipped_image,max_delta=0.8)    #将左右翻转好的图片进行随机亮度调整，使用tf.image.random.brightness()函数
        ...
        float_image=tf.image.per_image_standardization(adjusted_contrast)    #精选标准化图片操作，tf.image.per_image_standarization()函数是对每一个像素减去平均值并除以像素平方差
        ...
        images_train,labels_train=tf.train.shuffle_batch([float_image,read_input.label],batch_size=batch_size,
                                ...
                                #使用tf.train.shuffle batch()函数随机产生一个batch的image和label
        ...
        images_test,label_test=tf.train.batch([float_image,read_input.label],
                                              ...
                                              capacity=min_queue_examples + 3 * batch_size)
                              #这里使用batch（）函数代替tf.train.shuffle_batchu()函数
